...
if(u1=='func'){ff=async function(){let o=proj.val['func'];
let m=qq.reg.get('panels').edit;m.innerText=o[this.u];m.elem=this;m.nm=this.u}}else if(u1=='els'){ff=async function(){let ht=proj.val[u1][this.u]['html'];let f=qq.reg.get('htmltotable');let d=qq.reg.get('panels').propertes;f(ht,d);}};//alert(f);
